...
`arabtex-granada`. The historical files below `texinput/`, `tfm/`, `type1/`,
...
These files are stored under a private path so `arabtex-granada` can operate
...
The three files whose names begin with `arabtex-granada-` are renamed derived
...
The `arabtex-granada.sty` entry package, Granada transcription convention,
